let
  dagon_user =
    "ssh-rsa  michal_atlas@dagon";
  dagon_sys =
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBBXTuSh8QKK5Zm6j2m67Xe1H6PHQ0xd6P6o92+Ur3hz root@dagon";
  dagon = [ dagon_user dagon_sys ];
  hydra_sys =
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHp5iggvaDE3rbzNRteh6ckWVKicSFbTYMp1SeQYnRQ2 root@hydra";
  hydra_user =
    "ssh-rsa  michal_atlas@hydra";
  hydra = [ hydra_user hydra_sys ];
  leviathan_sys =
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINxxlUcmJvrFYOyzWovlFVGN3ovl57mVDE5XSAMSpkf8 root@leviathan";
  leviathan = [ leviathan_sys dagon_user ];
in {
  "secrets/yggdrasil/dagon.json".publicKeys = dagon;
  "secrets/wireguard/dagon".publicKeys = dagon;
  "secrets/yggdrasil/hydra.json".publicKeys = hydra;
  "secrets/wireguard/hydra".publicKeys = hydra;
  "secrets/yggdrasil/leviathan.json".publicKeys = leviathan;
  "secrets/wireguard/leviathan".publicKeys = leviathan;
  "secrets/fit-vpn".publicKeys = dagon ++ hydra ++ leviathan;
  "secrets/fit-mount".publicKeys = dagon ++ hydra ++ leviathan;
}
